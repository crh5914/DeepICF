...
from nDataset import Dataset
import nBatch_gen as data
import nEvaluate as evaluate
...
class DeepICF:
    ...
    def _create_placeholders(self):
        with tf.name_scope("input_data"):
            ...
            self.backet = tf.placeholder(tf.int32, shape=[None, None])  # the index of users
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):  # The embedding initialization is unknown now
            ...
            self.embedding_U = tf.Variable(tf.truncated_normal(shape=[self.num_users,self.embedding_size],mean=0.0,stddev=0.1),name='embedding_U',dtype=tf.float32)
            self.item_bias = tf.Variable(tf.zeros(self.num_items), name='item_bias')
            self.user_bias = tf.Variable(tf.zeros(self.num_users),name='user_bias')
            self.weights = {
                'out': tf.Variable(
            ...
            n_hidden_0 = 3*self.embedding_size
    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            self.embedding_u = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_U,self.user_input),1) # (?,k)
            self.embedding_p = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q_, self.backet), 1)  # (?, k)
            self.embedding_q = tf.reduce_sum(tf.nn.embedding_lookup(self.embedding_Q, self.item_input), 1)  # (?, k)
            self.bias_i = tf.nn.embedding_lookup(self.item_bias, self.item_input)  # (?, 1)
            self.bias_u = tf.nn.embedding_lookup(self.user_bias,self.user_input)
...
def training(flag, model, dataset, epochs, num_negatives):
    ...
    saver = tf.train.Saver([model.embedding_U,model.c1, model.embedding_Q, model.item_bias,model.user_bias])
    ...
    with tf.Session() as sess:
        ...
        if flag != 0:
            ...
            p_e_U,p_c1, p_e_Q, p_b_i,p_b_u = sess.run([model.embedding_U,model.c1, model.embedding_Q, model.item_bias,model.user_bias])
            ...
            model.item_bias = tf.Variable(p_b_i, dtype=tf.float32, trainable=True, name='item_bias')
            model.embedding_U = tf.Variable(p_e_U,dtype=tf.float32,trainable=True,name='embedding_U')
            model.user_bias = tf.Variable(p_b_u, dtype=tf.float32, trainable=True, name='user_bias')
...
def training_batch(batch_index, model, sess, batches):
    for index in batch_index:
        user_input, backet,num_idx, item_input, labels = data.batch_gen(batches, index)
        feed_dict = {model.user_input: user_input[:,None], model.backet:backet,model.num_idx: num_idx[:, None],
...
if __name__ == '__main__':
    ...
    model = DeepICF(dataset.num_users,dataset.num_items, args)
    model.build_graph()
